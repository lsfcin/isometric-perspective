...
function computeVisibilityDrawPlan(controlledToken) {
  ...
  plan.debugTokens.push({ gx, gy, px: controlledToken.center.x, py: controlledToken.center.y });
    ...
    // Determine occlusion against tiles by grid rule (token occluded iff gx > tx AND gy < ty)
    const occludingTiles = tileEntries.filter(te => gx > te.gx && gy < te.gy);
    if (occludingTiles.length) {
      const best = selectNearestGridOccluder(occludingTiles, gx, gy);
      if (best) {
        const occ = cloneTileSprite(best.tile.mesh, best.walls);
  ...
  for (const token of canvas.tokens.placeables) {
    ...
    // Grid-based occlusion: token is occluded by a tile iff gx > tile.gx AND gy < tile.gy
    const occludingTiles = tileEntries.filter(te => gx > te.gx && gy < te.gy);
    if (occludingTiles.length) {
      const best = selectNearestGridOccluder(occludingTiles, gx, gy);
      if (best) {
